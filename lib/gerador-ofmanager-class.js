...
const ArtefatoSaida = require('../models/artefato-saida-ofmanager')
const SaidaVO = require('../models/saida-vo')
const GeradorClass = require('./gerador-class')

const TIPO_ARTEFATO = require('./constants').TIPO_ARTEFATO

const geradorUtil = require('../util/gerador-util')

module.exports = class GeradorOfManagerClass extends GeradorClass {
  ...
  agruparArtefatoPorTarefa(listaArquivo) {
    ...
    return listaArquivo.reduce((listaTarefaRetorno, arquivoReduce) => {
      ...
      // TODO - Colocar essa condição no gerador
      if (arquivoReduce.commit.isTipoAlteracaoModificacao() ||
        arquivoReduce.commit.isTipoAlteracaoAdicionar() ||
        super.isMostrarRenomeados(this.params, arquivoReduce.commit) ||
        super.isMostrarDeletados(this.params, arquivoReduce.commit)) {
