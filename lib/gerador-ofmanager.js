  ...
  */
  ...
  function agruparArtefatoPorTarefa(listaArquivo) {
    ...
    return listaArquivo.reduce((listaRetorno, arquivoReduce) => {
      ...
      } else if (listaRetorno.length > 0) {

        let tarefaEncontrada = listaRetorno.find(tarefa =>
          tarefa.numeroTarefa === tarefa.tarefaEncontrada)

        if (tarefaEncontrada) {

          let artefatoEncontrado = tarefaEncontrada.listaTarefa.find(tarefa =>
          ...
          listaRetorno.push(novoArtefato)
        }
      }
